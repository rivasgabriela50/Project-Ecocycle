...
function updateMaterialCard(matKey, confidence) {
  ...
  document.getElementById('resIcon').innerText = mat.icon;
  document.getElementById('resMaterial').innerText = mat.name;
  document.getElementById('resDetail').innerText = `Confianza: ${Math.round(confidence * 100)}% · Contenedor #${mat.bin}`;
...
// Reemplaza la función addBubble en tu script2.js con esta:
function addBubble(text, type) {
  const chatArea = document.getElementById('chatArea');
  if (!chatArea) return;

  const div = document.createElement('div');
  // Usa las clases nativas definidas en tu CSS
...
// Opcional: agrega el evento 'Enter' dentro de DOMContentLoaded
document.addEventListener('DOMContentLoaded', () => {
  startCamera();
  fetchStats();
  setInterval(fetchStats, 5000);
